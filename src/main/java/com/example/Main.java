...
//            zipParameters.setCompressionMethod(CompressionMethod.STORE);
	...
	private static File genTheZipFile(String hashParent) {
		...
		if (zipFile.exists()) {
			if (hashParent.contains("(")) {
				Integer index = hashParent.indexOf("(") + 1;
				Integer version = hashParent.indexOf(index);
				Integer newVersion = version + 1;
				hashParent = hashParent.replace(version+"",newVersion+"");
				genTheZipFile(hashParent);
			}else{
				hashParent=zipPath+"/"+hashParent+"("+1+")"+".zip";
				genTheZipFile(hashParent);
			}
		}
